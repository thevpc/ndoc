...
public class NTxFunctionArgsImpl implements NTxFunctionArgs {
    ...
    public NTxFunctionArgsImpl(NElement[] callArgs, NTxNode node, NTxEngine engine, NTxVarProvider varProvider) {
        this(Arrays.stream(callArgs).map(x -> new NTxFunctionArgImpl(x, node,engine,varProvider)).toArray(NTxFunctionArg[]::new));
    }
    public NTxFunctionArgsImpl(List<NElement> callArgs, NTxNode node, NTxEngine engine, NTxVarProvider varProvider) {
