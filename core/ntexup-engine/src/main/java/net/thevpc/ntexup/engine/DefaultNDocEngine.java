...
import net.thevpc.ntexup.api.engine.NDocTemplateInfo;
...
import net.thevpc.ntexup.engine.templates.NDocTemplateInfoImpl;
...
import net.thevpc.ntexup.engine.tools.MyNDocEngineTools;
...
public class DefaultNDocEngine implements NDocEngine {
    ...
    @Override
    public List<NTxNode> compilePageNode(NTxNode node, NDocument document) {
...
//                            }
    ...
    @Override
    public NOptional<NTxItem> loadNode(NTxNode into, NPath path, NDocument document) {
        if (path.exists()) {
            if (path.isRegularFile()) {
                NDocResource source = NDocResourceFactory.of(path);
    ...
    private NOptional<NTxItem> loadNode0(NTxNode into, NPath path, NDocument document) {
        NDocResource source = NDocResourceFactory.of(path);
