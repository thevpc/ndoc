...
public class NDocQuadCurveBuilder implements NDocNodeBuilder {
    ...
    public void build(NTxNodeCustomBuilderContext builderContext) {
        builderContext.id(NTxNodeType.QUAD_CURVE)
                .parseParam().named(NTxPropName.FROM, NTxPropName.TO, NTxPropName.START_ARROW, NTxPropName.END_ARROW, NTxPropName.CTRL).then()
    ...
    public void renderMain(NTxNode p, NDocNodeRendererContext rendererContext, NTxNodeCustomBuilderContext builderContext) {
        ...
        NTxPoint2D to = NTxPoint.ofParent(NDocValue.ofProp(p, NTxPropName.TO).asHPoint2D().get()).valueHPoint2D(b, rendererContext.getGlobalBounds())
                .plus(translation);
        NTxPoint2D ctrl = NTxPoint.ofParent(NDocValue.ofProp(p, NTxPropName.CTRL).asHPoint2D().get()).valueHPoint2D(b, rendererContext.getGlobalBounds())
                .plus(translation);
