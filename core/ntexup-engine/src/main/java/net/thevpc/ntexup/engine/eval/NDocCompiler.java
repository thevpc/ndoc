...
public class NDocCompiler {
    ...
    public NTxDocumentLoadingResult compile(NTxDocument document0) {
        ...
        try {
            ...
            List<NTxItem> all = compileNodeTree(root, new NodeHierarchy(null, false, false, new NDocCompilePageContextImpl(engine, documentCopy)));
//            checkNode(root,null);
...
//                }
    ...
    private NTxNodeDef compileNodeDef(NTxNodeDef item, NodeHierarchy h) {
        ...
        return new NTxNodeDefImpl(
                ...
                new NDocItemBag(body).compressToNodes(h.isInPage, item.source()).toArray(new NTxNode[0]),
                item.source()
        );
    }
    ...
    private static class NodeHierarchy {
        ...
        public NodeHierarchy(NTxNode parent, boolean processPages, boolean isInPage, NDocCompilePageContext context) {
            this.parent = parent;
        ...
        public NodeHierarchy(NTxNode parent, NTxNodeDef def, NTxNode hierarchy, boolean processPages, boolean isInPage, NDocCompilePageContext context) {
            this.parent = parent;
            this.def = def;
...
//            }
    ...
    private DefaultNDocNodeFactoryParseContext createParseContext(NElement element, NTxNode node, NDocResource resource, NodeHierarchy compilePageContext) {
...
//                NDocNode currNode = NDocUtils.firstNodeUp(node.parent());
                ...
                if (dd.isPresent()) {
                    ...
                    NTxNode defaultNDocNode = DefaultNTxNode.ofText((NMsg.ofC("undefined node %s", uid).toString()));
            ...
            if (!h.isInPage || (h.isInPage && h.processPages)) {
                ...
                for (NElement o : b) {
                    for (NElement e : c.getBody()) {
                        NTxNode nDocNode = (NTxNode) engine.newNode(e, createParseContext(e, node, node.source(), h)).get();
