...
public class NDocLineBuilder implements NDocNodeCustomBuilder {
    ...
    public void renderMain(NDocNode p, NDocNodeRendererContext rendererContext, NDocNodeCustomBuilderContext builderContext) {
        ...
        NDocPoint2D from = NDocPoint.ofParent(NDocObjEx.ofProp(p, NDocPropName.FROM).asHPoint2D().orElse(new NDocPoint2D(0, 0))).valueHPoint2D(b, rendererContext.getGlobalBounds())
                .plus(translation);
        NDocPoint2D to = NDocPoint.ofParent(NDocObjEx.ofProp(p, NDocPropName.TO).asHPoint2D().orElse(new NDocPoint2D(0, 0))).valueHPoint2D(b, rendererContext.getGlobalBounds())
                .plus(translation);
