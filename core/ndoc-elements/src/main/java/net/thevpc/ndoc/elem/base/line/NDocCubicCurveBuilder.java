...
public class NDocCubicCurveBuilder implements NDocNodeCustomBuilder {
    ...
    public void renderMain(NDocNode p, NDocNodeRendererContext rendererContext, NDocNodeCustomBuilderContext builderContext) {
        ...
        NDocPoint2D to = NDocPoint.ofParent(NDocObjEx.ofProp(p, NDocPropName.TO).asHPoint2D().get()).valueHPoint2D(b, rendererContext.getGlobalBounds())
                .plus(translation);
        NDocPoint2D ctrl1 = NDocPoint.ofParent(NDocObjEx.ofProp(p, NDocPropName.CTRL1).asHPoint2D().get()).valueHPoint2D(b, rendererContext.getGlobalBounds())
                .plus(translation);
        NDocPoint2D ctrl2 = NDocPoint.ofParent(NDocObjEx.ofProp(p, NDocPropName.CTRL2).asHPoint2D().get()).valueHPoint2D(b, rendererContext.getGlobalBounds())
                .plus(translation);
