    ...
    protected boolean processArgument(ParseArgumentInfo info) {
        switch (info.currentArg.type()) {
            ...
            case NAME: {
                ...
                if (u.isPresent()) {
                    String pid = NDocUtils.uid(u.get());
                    if (HParserUtils.isCommonStyleProperty(pid)) {
                        //will be processed later
                        info.node.setProperty(pid, NDocUtils.addCompilerDeclarationPath(NElement.ofTrue(),info.source()));
                        return true;
                    }
                }
                break;
...
//                NDocObjEx ee = NDocObjEx.of(tsonElement);
                ...
                info.id = id;
                info.uid = NDocUtils.uid(id);
                info.tsonElement = NDocUtils.addCompilerDeclarationPath(tsonElement, info.context.source());
                info.node = p;
                List<NElement> body;
                switch (tsonElement.type()) {
                    case UPLET:
                ...
                if(info.arguments!=null) {
                    NElement[] arguments = info.arguments;
                    for (int i = 0; i < arguments.length; i++) {
                        info.arguments[i] = NDocUtils.addCompilerDeclarationPath(arguments[i], info.context.source());
                    }
                }
