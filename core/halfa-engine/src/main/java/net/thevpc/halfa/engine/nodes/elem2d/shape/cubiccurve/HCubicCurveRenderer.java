package net.thevpc.halfa.engine.nodes.elem2d.shape.cubiccurve;
...
public class HCubicCurveRenderer extends HNodeRendererBase {
    HProperties defaultStyles = new HProperties();

    public HCubicCurveRenderer() {
    ...
    public void renderMain(HNode p, HNodeRendererContext ctx) {
        ...
        HPoint2D to = HPoint.ofParent(ObjEx.ofProp(p, HPropName.TO).asHPoint2D().get()).valueHPoint2D(b, ctx.getGlobalBounds())
                .plus(translation);
        HPoint2D ctrl1 = HPoint.ofParent(ObjEx.ofProp(p, HPropName.CTRL1).asHPoint2D().get()).valueHPoint2D(b, ctx.getGlobalBounds())
                .plus(translation);
        HPoint2D ctrl2 = HPoint.ofParent(ObjEx.ofProp(p, HPropName.CTRL2).asHPoint2D().get()).valueHPoint2D(b, ctx.getGlobalBounds())
                .plus(translation);
