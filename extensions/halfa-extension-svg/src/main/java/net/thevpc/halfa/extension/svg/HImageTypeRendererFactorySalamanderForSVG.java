package net.thevpc.halfa.extension.svg;
...
public class HImageTypeRendererFactorySalamanderForSVG implements HImageTypeRendererFactory {
    @Override
    public NCallableSupport<HGraphicsImageDrawer> resolveRenderer(NPath path, HImageOptions options, HGraphics graphics) {
        if (path.getName().toLowerCase().endsWith(".svg")) {
            return NCallableSupport.of(10,
                    () -> {
                        return new SvgHImageByTypeRenderer(path);
                    }
            );
        }
    ...
    private static class SvgHImageByTypeRenderer implements HGraphicsImageDrawer {
        private final NPath path;

        public SvgHImageByTypeRenderer(NPath path) {
            this.path = path;
        }
