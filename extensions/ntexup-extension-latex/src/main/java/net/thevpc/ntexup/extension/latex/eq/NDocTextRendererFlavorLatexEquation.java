...
public class NDocTextRendererFlavorLatexEquation implements NDocTextRendererFlavor {
    ...
    @Override
    public void buildText(String text, NDocTextOptions options, NTxNode p, NDocNodeRendererContext ctx, NDocTextRendererBuilder builder) {
        if (!text.isEmpty()) {
    ...
    public NDocTextRendererBuilder.ImagePainter createLatex(String tex, double fontSize, NDocTextOptions options, NTxNode p, NDocNodeRendererContext ctx) {
        TeXFormula formula;
        boolean error = false;
        try {
