...
public class NTxMyShapeBuilder implements NTxNodeBuilder {
    ...
    public void render(NTxNode p, NTxNodeRendererContext renderContext, NTxNodeCustomBuilderContext buildContext) {
        ...
        NTxPoint2D base = NTxValue.of(p.getPropertyValue("base")).asHPoint2D().orNull();
        if (base == null) {
            Double base3 = NTxValue.of(p.getPropertyValue("base")).asDouble().orNull();
            if (base3 == null) {
        ...
        NTxPoint2D hat = NTxValue.of(p.getPropertyValue("hat")).asHPoint2D().orNull();
        if (hat == null) {
            Double base3 = NTxValue.of(p.getPropertyValue("hat")).asDouble().orNull();
            if (base3 == null) {
