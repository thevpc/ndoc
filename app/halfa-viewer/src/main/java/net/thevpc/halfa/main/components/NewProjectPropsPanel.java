...
public class NewProjectPropsPanel extends JPanel {
    ...
    public NewProjectPropsPanel(ServiceHelper serviceHelper) {
        ...
        int row = 0;
        add(new JLabel("Template"), HadraUIHelper.forLabel(0, row));
        add(template = new JComboBox<>(), HadraUIHelper.forEditor(1, row));
        ...
        for (PropInfo s : propInfos) {
            ...
            add(new JLabel(s.label), HadraUIHelper.forLabel(0, row));
            add(s.field, HadraUIHelper.forEditor(1, row));
            row++;
        }
    }
